...
const sensitiveKeyPattern = /(password|secret|token|key|authorization|cookie|credential|signature|private|databaseUrl|connectionString)/i;
const sensitiveValuePattern = /(Bearer\s+[A-Za-z0-9._~+\/-]{12,}|-----BEGIN .*PRIVATE KEY-----|postgres(?:ql)?:\/\/[^\s]+|SUPABASE_SERVICE_ROLE_KEY\s*=)/i;
...
export function writeLog(level: LogLevel, message: string, context: LogContext = {}) {
  const payload = {
    level,
    message,
    service: context.service ?? "remotedesk-api",
