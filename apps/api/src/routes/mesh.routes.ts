...
router.get(
  ...
  asyncHandler<AuthedRequest>(async (req, res) => {
    ...
    const routes = await prisma.meshRoute.findMany({ where: { networkId: network.id }, orderBy: { createdAt: "desc" } });
...
const splitTunnelInput = z.object({
  kind: z.enum(["cidr", "domain", "app"]).default("cidr"),
  value: z.string().min(1).max(255),
  action: z.enum(["include", "exclude"]).default("include"),
...
router.get(
  "/split-tunnel/:nodeId",
  asyncHandler<AuthedRequest>(async (req, res) => {
    const node = await getOwnedNode(req.user!.id, req.params.nodeId);
    if (!node) return res.status(404).json({ success: false, message: "Node not found" });
    const rules = await prisma.meshSplitTunnelRule.findMany({ where: { nodeId: node.id }, orderBy: { createdAt: "asc" } });
...
router.post(
  ...
  asyncHandler<AuthedRequest>(async (req, res) => {
    const node = await getOwnedNode(req.user!.id, req.params.nodeId);
    if (!node) return res.status(404).json({ success: false, message: "Node not found" });
    const input = splitTunnelInput.safeParse(req.body);
    if (!input.success) return res.status(400).json({ success: false, message: "Invalid payload", issues: input.error.issues });
    const rule = await prisma.meshSplitTunnelRule.create({ data: { nodeId: node.id, ...input.data } });
    res.status(201).json({ success: true, data: rule });
  })
);

router.delete(
  "/split-tunnel/:nodeId/:ruleId",
  asyncHandler<AuthedRequest>(async (req, res) => {
    const node = await getOwnedNode(req.user!.id, req.params.nodeId);
    if (!node) return res.status(404).json({ success: false, message: "Node not found" });
    const rule = await prisma.meshSplitTunnelRule.findFirst({ where: { id: req.params.ruleId, nodeId: node.id } });
    if (!rule) return res.status(404).json({ success: false, message: "Rule not found" });
    await prisma.meshSplitTunnelRule.delete({ where: { id: rule.id } });
    res.json({ success: true });
  })
);
...
const multiHopInput = z.object({
  sourceNodeId: z.string().uuid(),
  targetNodeId: z.string().uuid(),
  hopNodeIds: z.array(z.string().uuid()).max(4),
  enabled: z.boolean().default(true)
});
...
router.get(
  ...
  asyncHandler<AuthedRequest>(async (req, res) => {
    const network = await ensureNetwork(req.user!.id);
    const routes = await prisma.meshMultiHopRoute.findMany({ where: { networkId: network.id }, orderBy: { createdAt: "desc" } });
    res.json({ success: true, data: routes });
  })
);
...
router.post(
  ...
  asyncHandler<AuthedRequest>(async (req, res) => {
    const network = await ensureNetwork(req.user!.id);
    const input = multiHopInput.safeParse(req.body);
    if (!input.success) return res.status(400).json({ success: false, message: "Invalid payload", issues: input.error.issues });
    const allNodeIds = [input.data.sourceNodeId, ...input.data.hopNodeIds, input.data.targetNodeId];
    if (new Set(allNodeIds).size !== allNodeIds.length) return res.status(400).json({ success: false, message: "Multi-hop route contains a loop" });
    const count = await prisma.meshNode.count({ where: { networkId: network.id, id: { in: allNodeIds } } });
    if (count !== allNodeIds.length) return res.status(400).json({ success: false, message: "All route nodes must belong to this network" });
    const route = await prisma.meshMultiHopRoute.upsert({
      where: { sourceNodeId_targetNodeId: { sourceNodeId: input.data.sourceNodeId, targetNodeId: input.data.targetNodeId } },
      update: { hopNodeIds: input.data.hopNodeIds, enabled: input.data.enabled },
      create: { networkId: network.id, ...input.data }
    });
    res.status(201).json({ success: true, data: route });
  })
);
...
router.get(
  ...
  asyncHandler<AuthedRequest>(async (req, res) => {
    const network = await ensureNetwork(req.user!.id);
    const session = await prisma.meshHotspotSession.findFirst({ where: { networkId: network.id }, orderBy: { createdAt: "desc" } });
...
router.post(
  ...
  asyncHandler<AuthedRequest>(async (req, res) => {
    const network = await ensureNetwork(req.user!.id);
    const node = await prisma.meshNode.findFirst({ where: { id: req.params.nodeId, networkId: network.id } });
    if (!node) return res.status(404).json({ success: false, message: "Node not found" });
    const enabled = Boolean(req.body?.enabled);
    const session = await prisma.meshHotspotSession.upsert({
      where: { id: String(req.body?.sessionId ?? "00000000-0000-0000-0000-000000000000") },
      update: { enabled, providerNodeId: node.id, startedAt: enabled ? new Date() : undefined, stoppedAt: enabled ? undefined : new Date() },
      create: { networkId: network.id, providerNodeId: node.id, enabled, startedAt: enabled ? new Date() : undefined, stoppedAt: enabled ? undefined : new Date() }
    });
    res.json({ success: true, data: session });
  })
);
...
// ------------------------------------------------------------------
// STUN/TURN relay discovery
// ------------------------------------------------------------------

router.get(
  "/relays",
  asyncHandler<AuthedRequest>(async (_req, res) => {
    const relayUrl = process.env.MESH_TURN_URL ?? process.env.MESH_RELAY_URL ?? null;
    const region = process.env.MESH_TURN_REGION ?? "auto";
    res.json({ success: true, data: { stun: process.env.MESH_STUN_URL ?? "stun:stun.l.google.com:19302", turn: relayUrl ? { url: relayUrl, region } : null } });
  })
);

// ------------------------------------------------------------------
// Port-forward deletion
// ------------------------------------------------------------------

router.delete(
  "/port-forwards/:forwardId",
  asyncHandler<AuthedRequest>(async (req, res) => {
    const network = await ensureNetwork(req.user!.id);
    const forward = await prisma.meshPortForward.findFirst({ where: { id: req.params.forwardId, networkId: network.id } });
    if (!forward) return res.status(404).json({ success: false, message: "Port forward not found" });
    await prisma.meshPortForward.delete({ where: { id: forward.id } });
    res.json({ success: true });
  })
);
